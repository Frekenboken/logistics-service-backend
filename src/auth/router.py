...
from src.auth.security import security, access_token_required
...
async def get_current_user_profile(payload: TokenPayload = Depends(security.access_token_required),
                                   session: AsyncSession = Depends(get_session)):
